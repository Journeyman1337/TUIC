...
void key_callback(TuiWindow window, TuiKeyboardKey key, int scancode, TuiButtonState state, TuiKeyboardMod mods)
{
	...
	{
		TuiBoolean raw_mouse_motion_enabled = tuiGetRawMouseMotionEnabled(window);
		tuiSetRawMouseMotionEnabled(!raw_mouse_motion_enabled);
...
int main()
{
	...
	tuiWindodwSetCursorMoveCallback(window, cursor_move);
	
	printf("Press spacebar to enable or disable raw mouse motion.\nRaw mouse motion disabled.\n");
